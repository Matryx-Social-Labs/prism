...
Prism has no photographs of its own. An outlet's photograph appears only as a **credited link preview of that outlet's report**: the "From the reports" rail under a record's header (one horizontal snap-scroll strip of 232px tiles at every width, labelled "From the reports · N photos", prev/next on desktop; never a grid — eight tall tiles pushed the record off the first screen) and a 64px thumbnail on a report card. Every tile opens the report it came from; the outlet's icon and "Photo: <outlet>" sit on the image itself (no badge: the tile is the link), and the alt text says whose photo it is. Images are hotlinked with `referrerpolicy=no-referrer`, never proxied or resized by us, never the share card, never the JSON-LD image, never a row's hero. `NEXT_PUBLIC_REPORT_IMAGES=0` removes every one of them: the right to even this much is not settled in India (legal note, 2026-09-18), and the design must survive without them.
