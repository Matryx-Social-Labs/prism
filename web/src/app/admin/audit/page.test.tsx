...
describe("the audit timeline", () => {
  it("groups changes by IST day, in words, with the code and details kept", async () => {
    render(<AuditPage />);
    const first = within(await screen.findByRole("region", { name: "Thu 24 Sept · IST" }));
    expect(first.getByText("1 CHANGE")).toBeInTheDocument();
    expect(first.getByText(/Filled in a batch's languages/)).toBeInTheDocument();
    expect(first.getByText("batch.languages")).toBeInTheDocument();
    expect(first.getByText("f@example.test")).toBeInTheDocument();
    expect(first.getByText('gated {"hi":3}')).toBeInTheDocument();
    expect(within(screen.getByRole("region", { name: "Wed 23 Sept · IST" })).getByText(/Changed a labeller's status/)).toBeInTheDocument();
    expect(screen.getByRole("region", { name: "Tue 22 Sept · IST" })).toBeInTheDocument();
  });

  it("filters by area and counts each", async () => {
    render(<AuditPage />);
    await userEvent.click(await screen.findByRole("radio", { name: /Labellers/ }));
    ...
    expect(screen.getByRole("radio", { name: /Batches/ })).toHaveTextContent("2");
  });
});
