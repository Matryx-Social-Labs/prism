...
// Ported from pmndrs/examples demos/nextjs-prism (MIT) — src/components/Rainbow.jsx
// Spectral gradient based on:
//   "Improving the Rainbow" by Alan Zucconi
//   "More accurate Iridescence" by Julia Poo (shadertoy ltKcWh)
// Original shader written by https://twitter.com/shuding_
...
export type RainbowMaterialImpl = THREE.ShaderMaterial & {
  time: number;
  speed: number;
  fade: number;
  startRadius: number;
...
const RainbowMaterial = shaderMaterial(
  ...
  /* glsl */ ` varying vec2 vUv;
    ...
    vec3 _saturate3 (vec3 x) {
      return min(vec3(1.,1.,1.), max(vec3(0.,0.,0.),x));
    }

    vec3 bump3y(vec3 x, vec3 yoffset) {
      vec3 y = vec3(1.,1.,1.) - x * x;
      y = _saturate3(y-yoffset);
      return y;
    }
...
declare module "@react-three/fiber" {
  interface ThreeElements {
    rainbowMaterial: ThreeElement<typeof THREE.ShaderMaterial> & {
      fade?: number;
      speed?: number;
      startRadius?: number;
      endRadius?: number;
...
export const Rainbow = forwardRef<
  THREE.Mesh,
  { startRadius?: number; endRadius?: number; fade?: number }
>(({ startRadius = 0, endRadius = 0.5, fade = 0.25, ...props }, fRef) => {
  const material = useRef<RainbowMaterialImpl>(null);
  const { width, height } = useThree((state) => state.viewport);
  // the rainbow plane must be long enough to reach every screen corner
  const length = Math.hypot(width, height) + 1.5;
  useFrame((_, delta) => {
    if (material.current) material.current.time += delta * material.current.speed;
  });
  return (
    <mesh ref={fRef} scale={[length, length, 1]} {...props}>
      ...
      <rainbowMaterial
        ref={material}
        key={(RainbowMaterial as unknown as { key: string }).key}
        fade={fade}
