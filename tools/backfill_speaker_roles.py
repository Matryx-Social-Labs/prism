...
class RoleOut(BaseModel):
    speaker: str
    role: str | None = Field(default=None, description="Their title, office or standing exactly as the article gives it, in English; null if it does not")
...
SYSTEM = (
    ...
    "title, office or standing the ARTICLE gives that speaker (e.g. 'Vice President of the United States', "
    "'Kerala Chief Minister', 'MEA spokesperson', 'senior police officer'), in English, as briefly as the article "
    "puts it. If the article does not say who they are, return null. Never guess from general knowledge."
)
...
WHERE jsonb_typeof(e.shared_fields->'claims') = 'array'
  ...
  AND EXISTS (SELECT 1 FROM jsonb_array_elements(e.shared_fields->'claims') c WHERE NOT (c ? 'speaker_role'))
...
async def main() -> int:
    ...
    async with get_session_factory()() as session:
        rows = (await session.execute(text(SELECT), {"days": args.days})).mappings().all()
    ...
    async def one(row):
        ...
        by = {r.speaker.strip().casefold(): faithful_role(r.role, flat) for r in out.roles}
        if args.limit:
            for r in out.roles:
                print(f"  {r.speaker!r}: {r.role!r} -> {by[r.speaker.strip().casefold()]!r}")
        for c in claims:
            if isinstance(c, dict) and "speaker_role" not in c:
