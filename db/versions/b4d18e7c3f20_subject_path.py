...
prefix LIKE, and prefix LIKE is the only query this column exists for. It is
built CONCURRENTLY for the reason migration 51de411d824c records — `events` is
written continuously, and a lock here stalls the deploy's own `alembic upgrade
head` until the API fails its healthcheck.
"""
...
def upgrade() -> None:
    ...
    # `events` is written continuously by the ingestion worker, and a plain
    # CREATE INDEX takes a lock that fights those writes — the build stalls, and
    # because the deploy start command runs `alembic upgrade head`, a stalled
    # build means the API never boots and the healthcheck fails. This repo has
    # already had that incident; migration 51de411d824c documents it. Hence
    # CONCURRENTLY, which cannot run inside a transaction, hence autocommit.
    # IF NOT EXISTS makes a re-run (or a hand-created index) idempotent.
    with op.get_context().autocommit_block():
        op.execute(
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS ix_events_subject_path "
            "ON events (subject_path text_pattern_ops)"
        )
        # The feed and every node page read "this branch, newest first".
        op.execute(
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS ix_events_subject_path_updated "
            "ON events (subject_path text_pattern_ops, last_updated_at DESC)"
        )


def downgrade() -> None:
    with op.get_context().autocommit_block():
        op.execute("DROP INDEX CONCURRENTLY IF EXISTS ix_events_subject_path_updated")
        op.execute("DROP INDEX CONCURRENTLY IF EXISTS ix_events_subject_path")
