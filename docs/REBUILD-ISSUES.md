...
| A26 | **Step 6 L2 rebuild: gate NOT met, and the blocker is the gold set.** | Best config (mE5 + entity∪mutual-kNN, product weight, CPM 0.020) scores **F1 0.4835 vs v1's 0.4541** with clean structure (max 25, none above) — then loses cross-validation: fold A 0.7778 vs baseline 0.6571, fold B **0.3404 vs 0.4848**. The baseline's own folds differ by 0.17, so the halves are not equally hard; at 45 stories / 103 positive pairs each fold holds ~50–70 pairs and cannot resolve a 6.5% difference. | **BLOCKED on gold-set expansion (plan P1-c)** — not on the algorithm |
| A27 | **The encoder was not the bottleneck for the story boundary.** | mE5 separates gold pairs far better than production mpnet — **AUC 0.9082 vs 0.7994**, same corpus, same source text — and clustered F1 barely moved (0.4457 → 0.4835, not CV-stable). Pairwise separation and boundary quality are different problems. | **MEASURED** — the step 3 model swap should be justified on cross-lingual coverage, not on L2 F1 |
| A28 | **Entities do candidate generation, not similarity — the plan's L2 premise is measured false.** | The entity graph proposes only **22 of 103** gold pairs *directly*, yet reaches recall 0.4563: over half its true positives come from clustering transitivity. Embeddings cannot replace it — true siblings are ~0.1% of 14.6M pairs, so separation needs FPR ≈ 1e-3 and AUC 0.91 is far short. Embedding-only agglomerative gave blobs of 638–1164; kNN *as a connectivity constraint* gave one blob of 5,334 (a kNN graph over a dense corpus is one connected component). | **MEASURED** — keep entities for sparsity; union with mutual-kNN is the promising direction |
| A29 | **Gold-set expansion prepared: 123 corpus-sampled seeds, 1,147 candidate pairs, awaiting labels.** | The existing set is self-described as "the CJP slice" — one partition group, all politics — which is why its CV folds are incomparable (v1 scores 0.6571 vs 0.4848 on them). New candidates are stratified across all ten sectors (politics 30, business 20, sports 15, other 12, cyber 10, finance 10, health 8, science 8, tech 5, entertainment 5) and proposed by **three independent signals** (embedding 538, actors 399, title 374) so no one algorithm's blind spots define the labels. The production partition is never consulted. | **READY FOR LABELLING** — `.cache/gold_review.html`, then `--compile` |
| A30 | **MEASURED: 82 of 128 stored tickers do not exist on NSE.** | 201 mentions across 103 events. **46 validate** (63 mentions), **82 do not** (138 mentions — 69% of what readers see). Two groups: real US listings with no Indian counterpart (META, NVDA, GOOGL, MSFT…), and outright fabrications — `HUL` (Hindustan Unilever is HINDUNILVR), **`ECB`** (a central bank, not a ticker), and the Yahoo Finance forms `UPL.NS`, `^BSESENSEX`, `^NSENIFTY`. | **MASTER BUILT** — `securities` + `tools.securities`, 2,559 NSE equities all with ISIN. Enforcement at write time still to wire |
