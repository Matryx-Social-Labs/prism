...
4. Service → **Variables** (use *References* for the DB/Redis ones):
   ...
   | `OPENROUTER_API_KEY` | your key from <https://openrouter.ai/keys> — chat models (`LLM_PROVIDER=openrouter`) and Jev decisions (`PRISM_MODEL_DECIDE`) both use it |
...
| Ask agent errors | Verify `OPENROUTER_API_KEY` and the credit balance (`GET /api/v1/admin/status`); a 402/429 puts the LLM client in a cooldown (`common/llm.py`) — look for `llm_quota_cooldown` in the logs |
