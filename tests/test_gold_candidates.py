...
def test_a_candidate_needs_more_than_half_of_the_definite_answers():
    answers = [(["b"], False, False), (["b"], False, False), ([], False, False)]
...
def test_an_exact_tie_does_not_carry():
    ...
    assert merge_votes([(["b"], False, False), ([], False, False)], ["b"])[0] == []
...
def test_unsure_is_dropped_from_the_denominator_not_counted_as_no():
    ...
    """
    answers = [(["b"], False, False), ([], True, False), ([], True, False)]
...
def test_a_task_everyone_was_unsure_about_yields_no_verdict():
    ...
    assert merge_votes([([], True, False), ([], True, False)], ["b"]) == ([], 0)
...
def test_a_single_labeller_is_reported_as_such():
    ...
    assert merge_votes([(["b"], False, False)], ["b", "c"]) == (["b"], 1)
...
def test_an_empty_selection_is_a_real_verdict_not_a_missing_one():
    ...
    assert merge_votes([([], False, False), ([], False, False)], ["b", "c"]) == ([], 2)
...
def test_only_candidates_offered_can_be_returned():
    ...
    assert merge_votes([(["zzz"], False, False)], ["b"])[0] == []


def test_a_skip_does_not_vote():
    """"I cannot read this language" is an answer about the LABELLER. Counting it
    would let someone who could not read the headlines decide the boundary — and
    72 of the first batch's 123 tasks are Kannada, Devanagari or Tamil, so this is
    the majority case, not an edge one."""
    answers = [(["b"], False, False), ([], False, True), ([], False, True)]
    assert merge_votes(answers, ["b"]) == (["b"], 1)


def test_a_task_everyone_skipped_yields_no_verdict():
    """Not "nobody linked these" — nobody could READ these. The task needs a
